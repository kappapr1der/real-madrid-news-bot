...
- Для больших матчей обычный пост накануне становится рубрикой «Перед свистком», а подтвержденный итог получает спокойную реакцию «Голос Бернабеу» в том же сообщении, без лишнего отдельного поста.
...
`white_frame.py` добавляет «Белый кадр» дважды в неделю: только настоящий снимок из официальной X-ленты «Реала» и только если подпись похожа на живой клубный момент, а не на рекламу. Нет нормального кадра или RSS-зеркало недоступно, слот молча пропускается. Формат не создает макеты и не повторяет этот же пост в дайджесте.

`la_fabrica.py` раз в неделю ищет только конкретные новости о Кастилье и академии: дебют, контракт, повышение, вызов, титул, аренда или переход. Обычная болтовня о кантере не пройдет; найденная новость тоже не вернется в следующий дайджест.

```env
WHITE_FRAME_ENABLED=true
WHITE_FRAME_DAYS=wednesday,sunday
WHITE_FRAME_TIME=18:30
LA_FABRICA_ENABLED=true
LA_FABRICA_DAYS=thursday
LA_FABRICA_TIME=12:00
```

Проверить оба формата без отправки в Telegram:

```bash
DRY_RUN=true python white_frame.py --force
DRY_RUN=true python la_fabrica.py --force
```

### Большие матчи

`MATCHDAY_MARQUEE_OPPONENTS` задает соперников для особого настроения, а `MATCHDAY_MARQUEE_COMPETITIONS` покрывает Лигу чемпионов. Для таких игр пост за сутки до начала выходит с заголовком «Перед свистком» и хэштегом `#ПередСвистком`; итоговый пост с подтвержденным счетом добавляет короткий «Голос Бернабеу» и `#ГолосБернабеу`. Это не новые дополнительные сообщения, а более живой вариант уже существующих матчевых постов.
