...
В день жеребьевки лиговой фазы ЛЧ бот включает одноразовый редакционный триггер: подтвержденная заметка о соперниках «Реала» получает отдельную шапку `Жеребьевка Лиги чемпионов` и единый отпечаток, поэтому несколько одинаковых публикаций не превратятся в серию молний. Дата задается через `UCL_DRAW_DATE`.
